...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/fastnas2023/3x-ui/fix/wireguard-client-binding/install.sh)
```

This fork's default installer pulls the `dev-latest` release from `fastnas2023/3x-ui`. You can also pass it explicitly:

```bash
bash <(curl -Ls https://raw.githubusercontent.com/fastnas2023/3x-ui/fix/wireguard-client-binding/install.sh) dev-latest
```
