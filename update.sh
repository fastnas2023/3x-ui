...
github_release_url() {
    printf 'https://github.com/%s/releases/%s' "${XUI_REPO:-MHSanaei/3x-ui}" "$1"
}

github_api_url() {
    printf 'https://api.github.com/repos/%s/%s' "${XUI_REPO:-MHSanaei/3x-ui}" "$1"
}
...
update_x-ui() {
    ...
    if [[ -n "${XUI_UPDATE_TAG}" ]]; then
        ...
        tag_version=$(${curl_bin} -Ls "$(github_api_url "releases/latest")" 2> /dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            _fail "ERROR: Failed to fetch x-ui version, it may be due to GitHub API restrictions, please try it later"
        fi
    fi
    echo -e "Got x-ui latest version: ${tag_version}, beginning the installation..."
    ${curl_bin} -fLRo ${xui_folder}-linux-$(arch).tar.gz "$(github_release_url "download/${tag_version}/x-ui-linux-$(arch).tar.gz")" 2> /dev/null
    if [[ $? -ne 0 ]]; then
    ...
    ${curl_bin} -fLRo /usr/bin/x-ui "$(github_raw_url "x-ui.sh")" > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        ...
        ${curl_bin} -fLRo /etc/init.d/x-ui "$(github_raw_url "x-ui.rc")" > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
        ...
        if [ -f "x-ui.service" ]; then
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service "$(github_raw_url "x-ui.service.debian")" > /dev/null 2>&1
                        ;;
                    arch | manjaro | parch)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service "$(github_raw_url "x-ui.service.arch")" > /dev/null 2>&1
                        ;;
                    *)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service "$(github_raw_url "x-ui.service.rhel")" > /dev/null 2>&1
                        ;;
                esac
